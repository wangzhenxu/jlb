...
function update(){
	...
	$('#updateClazz').ajaxSubmit(function(resp) {
		if (resp.s > 0) {
			location.href="/cmsSolution/class/list.action";
		} else {
...
function query(){
	...
	location.href="/cmsSolution/class/list.action?name="+name;
}
...
function deleteclass(pcid) {
	    ...
	    $(".pop_sure .mar").on("click",function(r) {
           if(r){
					$.get("/cmsSolution/class/delete.action",{pcid:pcid},function(resp) {
							if (resp.s > 0) {
							  location.href="/cmsSolution/class/list.action";
							} else {
...
$("#updateClazz").validationEngine("hide");
	...
	<div class="right">
    <div class="location">
     <div class="location01">您现在的位置是：首页 &gt; <strong>系统分类</strong></div>
    </div>
    <div class="sort">
     <div class="sort1">系统分类</div>
     <div class="query">
      <ul>
       <li>
       <span class="classify">系统分类：</span>
    ...
    <div class="form">
      <#if subject.isPermitted("cmsSolution:add")>
     ...
     <table width="100%"  border="1" align="left" cellpadding="0" cellspacing="0" bordercolor="#ffffff" style="border-collapse:collapse">
      <tr class="lan">
        <td width="31%" height="37" align="center" valign="middle" background="/images/erji_22.jpg"><strong>系统分类编号</strong></td>
        <td width="39%" height="37" align="center" valign="middle" background="/images/erji_22.jpg"><strong>系统分类</strong></td>
        <td width="30%" height="37" align="center" valign="middle" background="/images/erji_22.jpg"><strong>操 作</strong></td>
       </tr>
       <#list clazzes as clazz>
       <tr>
        <td align="center" class="hui">${clazz.cscId}</td>
        <td align="center" class="hui">${clazz.name}&nbsp;</td>
        <td align="center" class="hui">
        	<#if subject.isPermitted("cmsSolution:update")>
        		<a  href="javascript:toupdate('${clazz.name}','${clazz.cscId}');">修改</a>
        	</#if>
        	<#if subject.isPermitted("cmsSolution:delete")>
         		<a  href="javascript:deleteclass(${clazz.cscId});">删除</a></td>
         	</#if>
       </tr>
       </#list>
      </table>
	 ...
	 <#-- 分页栏 -->
     <@pageBar pager=pager url="/cmsSolution/class/list.action?name="+name join="&"></@pageBar>
    </div>
   </div>
  ...
  <div class="caidan">
   <div class="caidan1">
    ...
    <form id="ProductClazz"  action="/cmsSolution/class/add.action" method="post">
     <table width="100%" border="0">
      <tr>
        <td align="right" class="hui1">系统名称：</td>
 ...
 <div class="modal hide fade xin2">
  ...
  <div class="caidan">
   <div class="caidan1">
    ...
    <form id="updateClazz"  action="/cmsSolution/class/update.action" method="post">
     <table width="100%" border="0">
      <tr>
        <td align="right" class="hui1">系统名称：</td>
