...
$('#ProductClazz').ajaxSubmit(function(resp) {
		if (resp.s > 0) {
			location.href="/product/class/list.action";
		} else {
...
function query(){
	...
	location.href="/product/class/list.action?name="+name;
}
...
function deleteclass(pcid) {
	    ...
	    $(".pop_sure .mar").on("click",function(r) {
           if(r){
					$.get("/product/class/delete.action",{pcid:pcid},function(resp) {
							if (resp.s > 0) {
							  location.href="/product/class/list.action";
							} else {
...
$("#updateClazz").validationEngine("hide");
    ...
    <div class="form">
     ...
     <table width="100%"  border="1" align="left" cellpadding="0" cellspacing="0" bordercolor="#ffffff" style="border-collapse:collapse">
      <tr class="lan">
       ...
       <tr>
        <td align="center" class="hui">${clazz.pcId}</td>
        <td align="center" class="hui">${clazz.name}&nbsp;</td>
        <td align="center" class="hui">
        	<#if subject.isPermitted("productClass:edit")>
        		<a  href="javascript:toupdate('${clazz.name}','${clazz.pcId}');">修改</a>
        	</#if>
        	<#if subject.isPermitted("productClass:delete")>
         		<a  href="javascript:deleteclass(${clazz.pcId});">删除</a></td>
         	</#if>
       </tr>
       </#list>
      </table>
	 ...
	 <#-- 分页栏 -->
     <@pageBar pager=pager url="/product/class/list.action?name="+name join="&"></@pageBar>
    </div>
   </div>
  ...
  <div class="caidan">
   <div class="caidan1">
    ...
    <form id="ProductClazz"  action="/product/class/add.action" method="post">
 ...
 <div class="modal hide fade xin2">
  ...
  <div class="caidan">
   <div class="caidan1">
    ...
    <form id="updateClazz"  action="/product/class/update.action" method="post">
     <table width="100%" border="0">
      <tr>
        <td align="right" class="hui1">产品名称：</td>
