...
<div class="xin1">
	<form id="saveAccountForm" action="/tgAccount/edit.action" method="post">
     ...
     <table width="100%" border="0">
      ...
      <tr>
        ...
        <tr>
	        ...
	        <td align="left" valign="middle">
	          		<#list TgAccountType.values() as type>
	       ...
	       <tr id="auditPostion_id" <#if account.type??> <#if account.type==TgAccountType.TESTING.getCode()  || account.type==TgAccountType.PRODUCT_MANAGER.getCode()  ><#else> style="display:none;" </#if> </#if>   >
	        <td align="right" class="hui1">用户级别：</td>
	        <td align="left" valign="middle">
	        	<select id="accountLevel" name="accountLevel">
			    		 			<option value="" > 请选择 </option>
			    		    		<#list TgAccountLevelType.values() as c>
			    		 				 <option value="${c.code}"  <#if account.accountLevel??> <#if account.accountLevel?number==c.code> selected </#if> </#if>   > ${c.title} </option>
			 			 			</#list>
			    </select>
	          <span class='tip'></span>
	         </td>
