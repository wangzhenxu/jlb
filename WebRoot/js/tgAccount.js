function toAddAccount() {
	$.post("/tgAccount/toAddAccount.action", function(doc) {
...
//列表查询
 function query(){
	...
	location.href="/tgAccount/list.action?jsonParam="+jsonStr;
}

function toEditAccount(accountId) {
	$.post("/tgAccount/toEditAccount.action", {
		accountId : accountId
	}, function(doc) {
		var i=0;
		document.getElementById("accountModal").innerHTML=doc;
...
function toViewAccount(accountId) {
	$.post("/tgAccount/toViewAccount.action", {
		accountId : accountId
	}, function(doc) {
		document.getElementById("accountModal").innerHTML=doc;
		$("#accountModal").modal().css({
...
function addAccount() {
	...
	$form.ajaxSubmit(function(resp) {
		if (resp.s > 0) {
			closeModal();
			location.href="/tgAccount/list.action";
		} else {
			if(resp.s==-100301){
...
function accountTypeSelected(obj){
	if(obj.value==1 || obj.value==2){
		...
		$("#accountLevel").find("option").get(0).selected=true;
		$("#auditPostion_id").hide();
	}
}

modifyDeleteStatusUrl="/tgAccount/modifyDeleteStatus.action" //停用 或启用
