...
var accountExpandInfo = {
	//请求url
	listUrl:"/tgAccountExpandInfo/list.action", //列表地址
	toAddUrl:"/tgAccountExpandInfo/toAdd.action", //去添加页面地址
	addUrl:"/tgAccountExpandInfo/add.action", //添加地址
	toEditUrl:"/tgAccountExpandInfo/toEdit.action?id=", //去修改页面地址
	editUrl:"/tgAccountExpandInfo/edit.action", //去修改页面地址
	deleteUrl :"/tgAccountExpandInfo/delete.action?id=", //删除页面地址
	toViewUrl:"/tgAccountExpandInfo/toView.action?id=", //详细页面地址
	getByIdUrl : "/tgAccountExpandInfo/getById.action", //根据id查询对象
	modifyDeleteStatusUrl:"/tgAccountExpandInfo/modifyDeleteStatus.action", //停用 或启用
	...
	left_menu_selected_id : "tgAccountExpandInfo_list",   //左侧菜单选择id  
		   ...
		   var self = this;
			self.getById(self.expandId.val(),function (result){
				if (result.s > 0) {
					...
					//如果只有看的权限
					if(result.data.accountId!=accountIdT && accountType!=3 ){
						self.addBtn.hide();
					}
